...
| Cold-start gate | `MCP_MAX_CONCURRENT_SPAWNS` | `2` | how many servers may cold-start at once. Package-manager-backed servers (`npx`, `uvx`, `dnx`) share one cache, and starting several together can corrupt it — this queues them instead. `0` disables the gate |
