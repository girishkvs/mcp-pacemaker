...
const validEnvironment = (environment) => exact(environment, ['node', 'platform', 'release', 'arch', 'cpuModel',
...
export function validateTraceArtifact(artifact) {
  const environmentValid = validEnvironment(artifact?.environment) ||
    artifact?.environment === null && Array.isArray(artifact?.issues) &&
      artifact.issues.includes('metadata-unavailable');
...
export class PoolingTraceArtifact {
  concurrency(args, parallelism) {
    const flag = args.find((arg) => arg.startsWith('--test-concurrency='));
    const configured = flag ? Number(flag.split('=')[1]) : 0;
    const valid = Number.isInteger(configured) && configured >= 0 && configured <= 1024;
    if (!valid) throw new Error('Unsupported pooling trace concurrency.');
    // Node forwards zero to test children when the runner uses its default.
    return {
      effectiveConcurrency: configured || Math.max(parallelism - 1, 1),
      concurrencySource: configured ? 'explicit' : 'node-default',
    };
  }
  ...
  environment() {
    ...
    return {
      ...
      ...this.concurrency(process.execArgv, parallelism),
  ...
  collect(traceDirectory, configDirectory) {
    ...
    try {
      const candidate = this.environment();
      if (validEnvironment(candidate)) environment = candidate;
      else issues.add('metadata-unavailable');
    } catch {
      issues.add('metadata-unavailable');
    }
