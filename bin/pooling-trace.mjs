...
export class PoolingTrace {
  ...
  record(operation, event, extra) {
    ...
    try {
      ...
      this.failed = true;
      if (this.#owned) this.close();
    }
  }

  #append(line) {
    if (this.#descriptor === undefined) {
      this.#path = join(this.directory, `${process.pid}-${threadId}.jsonl`);
      this.#descriptor = fs.openSync(this.#path, 'wx', 0o600);
      this.#owned = true;
      this.#identity = fs.fstatSync(this.#descriptor, { bigint: true });
    }
    this.#checkOwned(this.#written);
    // A replacement after the check cannot redirect this write to another file.
    fs.appendFileSync(this.#descriptor, line);
    const written = this.#written + Buffer.byteLength(line);
    this.#checkOwned(written);
    this.#written = written;
  }

  #checkOwned(bytes) {
    const open = fs.fstatSync(this.#descriptor, { bigint: true });
    const named = fs.lstatSync(this.#path, { bigint: true });
    const owned = [open, named].every((stat) =>
      stat.isFile() && !stat.isSymbolicLink() && stat.nlink === 1n &&
      stat.dev === this.#identity.dev && stat.ino === this.#identity.ino &&
      stat.size === BigInt(bytes));
    if (!owned) throw new Error('Pooling trace file ownership changed.');
  }
...
if (poolingTrace.directory) process.once('exit', () => poolingTrace.close());
